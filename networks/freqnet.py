...
class FreqNet(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3, 4], num_classes=1, zero_init_residual=False):
        ...
        self.fc1 = nn.Linear(512, 512)  # Updated to output 512 features
    ...
    def hfreqWH(self, x, scale):
        ...
        if x.dim() == 5:
            ...
            for i in range(num_frames):
                frame = x[:, i, :, :, :]
                frame = torch.fft.fft2(frame, norm="ortho")
                frame = torch.fft.fftshift(frame, dim=[-2, -1])
                b, c, h, w = frame.shape
                frame[:, :, h // 2 - h // scale: h // 2 + h // scale, w // 2 - w // scale: w // 2 + w // scale] = 0.0
                frame = torch.fft.ifftshift(frame, dim=[-2, -1])
                frame = torch.fft.ifft2(frame, norm="ortho")
                frame = torch.real(frame)
                frame = F.relu(frame, inplace=True)
                output.append(frame)
    ...
    def forward(self, x):
        """
        Forward pass for the FreqNet model.

        Args:
            x (torch.Tensor): Input tensor of shape (B, num_frames, C, H, W).

        Returns:
            torch.Tensor: Output tensor of shape (B, 512).
        """
        if x.dim() == 4:
            x = x.unsqueeze(1)
